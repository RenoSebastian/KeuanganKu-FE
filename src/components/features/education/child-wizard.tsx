...
import {
  ...
  User, Baby, School, ArrowLeft,
  CheckCircle2, DollarSign, GraduationCap
} from "lucide-react";
import { financialService } from "@/services/financial.service";
import { EducationPayload, EducationStagePayload, EducationLevel } from "@/lib/types";
...
export function ChildWizard({ onSave, onCancel }: ChildWizardProps) {
  ...
  // Max age assumed ~25 for education planning context
  ...
  const handleGradeChange = (id: string, grade: number) => {
    ...
    // Jika bukan kelas 1, asumsi uang pangkal sudah lunas (opsional, bisa di-tweak sesuai kebutuhan)
    // Kecuali S2, karena S2 dianggap lumpsum kapanpun masuknya
    if (id !== "S2" && grade > 1) {
      setCosts(prev => ({
        ...prev,
        [id]: { ...prev[id], entry: 0 }
  ...
  // Helper untuk mapping string ID ke EducationLevel Type
  const mapLevel = (id: string): EducationLevel => {
    switch (id) {
      case "TK": return "TK";
      case "SD": return "SD";
      case "SMP": return "SMP";
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      selectedStageIds.forEach(id => {
        ...
        // Identifikasi S2 (Perlakuan Khusus)
        const isS2 = id === "S2";
        ...
        // 1. Tambahkan Uang Pangkal (Entry Fee) 
        // NOTE: Untuk S2, field 'entry' ini dianggap sebagai "Total Biaya Lumpsum"
        if (cost.entry > 0) {
          stagesPayload.push({
            level: mapLevel(id),
            costType: "ENTRY",
        ...
        // EXCEPTION: S2 tidak punya biaya periodik di form wizard ini (Single Cost Model)
          ...
          <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-100 overflow-y-auto pr-2 custom-scrollbar">
              {STAGES_DB.map((stage) => {
                ...
                // Simple logic: if child is already way past this stage
                const isTotallyPassed = age > (stage.entryAge + stage.duration + 2); // Buffer +2 years
          ...
          <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="max-h-100 overflow-y-auto pr-2 custom-scrollbar space-y-4 pb-2">
              {selectedStageIds.map((id) => {
                ...
                // Logic Disable Uang Pangkal (kecuali S2 yang selalu aktif karena single cost)
                const isEntryFeeDisabled = !isS2 && currentGrade > 1;
