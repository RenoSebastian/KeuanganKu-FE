...
export const generateCheckupPDF = (
  ...
  // --- REFACTOR UTAMA: Ekstraksi File alih-alih Unduh Paksa ---
  const pdfBlob = doc.output("blob");
  const filename = `Laporan_Checkup_${data.userProfile.name.replace(/\s+/g, "_")}.pdf`;

  // Konstruksi entitas File dengan parameter MIME Type yang spesifik
  const file = new File([pdfBlob], filename, {
    type: "application/pdf",
    lastModified: Date.now()
  });

  // Konstruksi URL memori sementara
  const url = URL.createObjectURL(file);

  // Mengembalikan data terstruktur ke caller di Layer UI
